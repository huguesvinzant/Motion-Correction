...
from VIBE.lib.utils.smooth_bbox import get_smooth_bbox_params, get_all_bbox_params
from VIBE.lib.data_utils.img_utils import get_single_image_crop_demo
from VIBE.lib.utils.geometry import rotation_matrix_to_angle_axis
from VIBE.lib.smplify.temporal_smplify import TemporalSMPLify
...
def video_to_images(video_dict_or_file, img_folder=None, return_info=False):
    if img_folder is None:
        img_folder = 'PoseCorrection/Data/Videos/Images'

    os.makedirs(img_folder, exist_ok=True)

    if isinstance(video_dict_or_file, dict):
        command = ['ffmpeg',
                   '-ss', video_dict_or_file['start'],
                   '-i', video_dict_or_file['file'],
                   '-t', video_dict_or_file['length'],
                   '-f', 'image2',
                   '-v', 'error',
                   f'{img_folder}/%06d.png']
    else:
        command = ['ffmpeg',
                   '-i', video_dict_or_file,
                   '-f', 'image2',
                   '-v', 'error',
                   f'{img_folder}/%06d.png']
...
def download_ckpt(outdir='VIBE/data/vibe_data', use_3dpw=False):
    os.makedirs(outdir, exist_ok=True)

    if use_3dpw:
        ckpt_file = 'VIBE/data/vibe_data/vibe_model_w_3dpw.pth.tar'
        url = 'https://www.dropbox.com/s/41ozgqorcp095ja/vibe_model_w_3dpw.pth.tar'
        if not os.path.isfile(ckpt_file):
            download_url(url=url, outdir=outdir)
    else:
        ckpt_file = 'VIBE/data/vibe_data/vibe_model_wo_3dpw.pth.tar'
